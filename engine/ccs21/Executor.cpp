...
// Replaced : klee/lib/Core/Executor.cpp
...
cl::opt<unsigned> MinSeedDepth(
    "min-seed-depth",
    cl::desc("Only save the negate state of this depth or more as a seed (default=0)"),
    cl::init(0),
    cl::cat(SeedingCat));

cl::opt<unsigned> MaxSeedDepth(
    "max-seed-depth",
    cl::desc("Only save the negate state of this depth or less as a seed (default=1000)"),
    cl::init(1000),
    cl::cat(SeedingCat));
...
Executor::fork(ExecutionState &current, ref<Expr> condition, bool isInternal) {
  ...
  if (res==Solver::True) {
    ...
    if (trueState == &current)
      current.forkedConstraints.push_back(condition);
    else
      trueState->forkedConstraints.push_back(condition);

    falseState->forkedConstraints.push_back(Expr::createIsZero(condition));
...
void Executor::run(ExecutionState &initialState) {
  ...
  if (usingSeeds) {
    ...
    std::set<ExecutionState*> copy_states(states.begin(), states.end());
    for (std::set<ExecutionState*>::iterator iter = copy_states.begin(); iter != copy_states.end(); iter++) {
      if ((*iter)->depth < MinSeedDepth || (*iter)->depth > MaxSeedDepth) {
        states.erase(*iter);
        ExecutionState *es = *iter;
        processTree->remove(es->ptreeNode);
      }
    }
...
static unsigned testCaseCounter = 1;
bool Executor::getSymbolicSolution(const ExecutionState &state,
  ...
  if (!state.forkedConstraints.empty()) {
    std::ostringstream idStream;
    idStream << std::setw(6) << std::setfill('0') << testCaseCounter;
    ...
    std::string filename = outDir + "/test" + testId + ".const";

    std::ofstream constraintFile(filename);
    if (constraintFile.is_open()) {
      constraintFile << "[";
      bool first = true;

      for (const auto &expr : state.forkedConstraints) {
        std::string buf;
        ...
        if (!first) constraintFile << ", ";
        first = false;
        constraintFile << "\"" << compact << "\"";
      }

      constraintFile << "]\n";
      constraintFile.close();
    } else {
      llvm::errs() << "[Warning] Failed to write constraint file: " << filename << "\n";
    }
  } else {
    llvm::errs() << "[Info] No forkedConstraints to write.\n";
  }
