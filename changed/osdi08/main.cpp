...
// Replaced : klee/tools/klee/main.cpp
...
llvm::cl::opt<bool> WriteArgConds(
    "write-arg-conds",
    llvm::cl::desc("Write per-argv constraints for each test"),
    llvm::cl::init(false));

llvm::cl::opt<std::string> ArgScoreFile(
    "arg-score-file",
    llvm::cl::desc("Path to argument score file (format: <arg> <score> per line)"),
    llvm::cl::init(""));

static std::map<std::string, double> ArgScores;
unsigned maxArgIndex = 0;
unsigned kSymFileArgIndex = maxArgIndex + 1;

namespace klee {
extern cl::opt<std::string> MaxTime;
size_t totalRuns = 0;
size_t fallbackRuns = 0;
}

/***/
std::map<std::string,double> filterCandidatesByConstraints(
    unsigned argIndex,
    const ExecutionState &state,
    const std::map<std::string,double> &allScores) {

  std::map<std::string,double> result = allScores;
  static const std::regex re(R"(Eq (\d+) \(Read w8 (\d+) arg0*(\d+)\))");
  ...
  for (auto &c : state.constraints) {
    ...
    if (std::regex_search(s, m, re)) {
      unsigned constVal = std::stoul(m[1].str());  // Eq <val>
      unsigned byteIdx  = std::stoul(m[2].str());  // index
      unsigned argIdx   = std::stoul(m[3].str());  // argN

      if (argIdx != argIndex) continue;

      std::map<std::string,double> filtered;
      for (auto &kv : result) {
        const std::string &cand = kv.first;
        if (byteIdx < cand.size()) {
          unsigned char ch = (unsigned char)cand[byteIdx];
          if (ch == constVal) {
            filtered[cand] = kv.second;
          }
        }
      }
      if (!filtered.empty()) {
...
void KleeHandler::processTestCase(const ExecutionState &state,
  ...
  if (!WriteNone) {
    ...
    if (success) {
      ...
      auto findArgObjectIndex =
        [](const std::vector<std::pair<std::string,std::vector<unsigned char>>> &objs,
           unsigned argIndex)->int {
          for (size_t i=0;i<objs.size();++i) {
            const std::string &name = objs[i].first; // "arg0", "arg01", ...
            if (name.size() >= 3 && name.rfind("arg",0)==0) {
              const char* p = name.c_str()+3; char* endp=nullptr;
              long v = strtol(p,&endp,10);
              if (endp && *endp=='\0' && v>=0 && (unsigned)v==argIndex) return (int)i;
            }
          }
          return -1;
        };
      auto writeCStringToObject = [](std::vector<unsigned char> &buf,
                                     const std::string &s) {
        const size_t n = buf.size(); if (!n) return;
        const size_t copyLen = (n>0) ? std::min(s.size(), n-1) : 0;
        for (size_t i=0;i<copyLen;++i) buf[i] = (unsigned char)s[i];
        if (copyLen < n) buf[copyLen] = 0;
        for (size_t i=copyLen+1;i<n;++i) buf[i] = 0;
      };

      auto cstrLen = [](const std::vector<unsigned char> &buf) -> size_t {
      ...
      struct Cond { unsigned argN; unsigned idx; unsigned val; bool isEq; };
      std::vector<Cond> parsedConds;
      if (!ArgScores.empty()) {
        static const std::regex re_eq (R"(\(Eq (\d+) \(Read w8 (\d+) arg0*([0-9]+)\)\))");
        static const std::regex re_neq(R"(\(Eq false \(Eq (\d+) \(Read w8 (\d+) arg0*([0-9]+)\)\)\))");
        for (auto it = state.constraints.begin(); it != state.constraints.end(); ++it) {
          std::string s; { llvm::raw_string_ostream rs(s); (*it)->print(rs); rs.flush(); }
          std::smatch m;
          if (std::regex_match(s, m, re_eq)) {
            parsedConds.push_back({(unsigned)std::stoul(m[3].str()),
                                   (unsigned)std::stoul(m[2].str()),
                                   (unsigned)std::stoul(m[1].str()),
                                   true});
          } else if (std::regex_match(s, m, re_neq)) {
            parsedConds.push_back({(unsigned)std::stoul(m[3].str()),
                                   (unsigned)std::stoul(m[2].str()),
                                   (unsigned)std::stoul(m[1].str()),
                                   false});
      ...
      std::map<std::string,double> remaining = ArgScores;
      std::map<unsigned,std::string> chosenForArg;
      std::set<unsigned> occupied;

      if (!remaining.empty()) {
        std::set<unsigned> targets; for (auto &c : parsedConds) targets.insert(c.argN);
        uint64_t seed = (uint64_t)std::time(nullptr)
                        ^ (uint64_t)(uintptr_t)this
                        ^ (uint64_t)id;
        std::mt19937_64 rng(seed);

        for (unsigned argIdx : targets) {
          if (occupied.count(argIdx)) continue;

          std::map<std::string,double> filtered;
          std::map<std::string,double> filteredStrict;

          for (auto &kv : remaining) {
            const std::string &cand = kv.first;
            bool ok = true;

            for (auto &c : parsedConds) {
              if (c.argN != argIdx) continue;
              if (c.idx >= cand.size()) { ok = false; break; }
              unsigned ch = (unsigned)(unsigned char)cand[c.idx];
              if (c.isEq && ch != c.val) { ok = false; break; }
              if (!c.isEq && ch == c.val) { ok = false; break; }
            }
            if (!ok) continue;

            auto toks = splitTokens(cand);

            bool fitCap = true;
            for (size_t k = 0; k < toks.size(); ++k) {
              unsigned target = argIdx + (unsigned)k;
              int oi = findArgObjectIndex(out, target);
              if (oi < 0) { fitCap = false; break; }
              size_t cap = out[(size_t)oi].second.size();
              if (toks[k].size() + 1 > cap) { fitCap = false; break; }
            }
            if (!fitCap) continue;

            bool fitCurrLen = true;
            for (size_t k = 0; k < toks.size(); ++k) {
              unsigned target = argIdx + (unsigned)k;
              int oi = findArgObjectIndex(out, target);
              if (oi < 0) { fitCurrLen = false; break; }
              size_t currLen = cstrLen(out[(size_t)oi].second);
              if (toks[k].size() > currLen) { fitCurrLen = false; break; }
            }

            filtered[cand] = kv.second;
            if (fitCurrLen) filteredStrict[cand] = kv.second;
          }
          const std::map<std::string,double> *useSet = nullptr;
          ++totalRuns;
          if (!filteredStrict.empty()) {
            useSet = &filteredStrict;
            ...
            if (coin(rng) < 0.0) {
              ...
              for (auto &kvAll : remaining) {
                const std::string &candAll = kvAll.first;
                bool fit = true;
                auto toks = splitTokens(candAll);
                for (size_t k = 0; k < toks.size(); ++k) {
                  unsigned target = argIdx + (unsigned)k;
                  int oi = findArgObjectIndex(out, target);
                  if (oi < 0) { fit = false; break; }
                  size_t cap = out[(size_t)oi].second.size();
                  if (toks[k].size() + 1 > cap) { fit = false; break; }
                }
                if (fit) pool.push_back(candAll);
              }

              if (!pool.empty()) {
                std::uniform_int_distribution<size_t> pick(0, pool.size() - 1);
                std::string chosen = pool[pick(rng)];

                chosenForArg[argIdx] = chosen;
                remaining.erase(chosen);

                auto toks = splitTokens(chosen);
                for (size_t k = 0; k < toks.size(); ++k)
                  occupied.insert(argIdx + (unsigned)k);
              }
            }
            continue;
          }

          std::vector<std::string> keys; keys.reserve(useSet->size());
          std::vector<double> w; w.reserve(useSet->size());
          double total = 0.0;

          for (auto &kv2 : *useSet) {
            keys.push_back(kv2.first);
            w.push_back(kv2.second);
            total += kv2.second;
          }

          std::uniform_real_distribution<double> uni(0.0, total);
          double r = uni(rng), acc = 0.0; std::string chosen;
          for (size_t i = 0; i < keys.size(); ++i) {
            acc += w[i];
            if (r <= acc) { chosen = keys[i]; break; }
          }
          if (chosen.empty()) chosen = keys.back();
        ...
        for (auto &kv : chosenForArg) {
          unsigned base = kv.first;
          auto toks = splitTokens(kv.second);
          for (size_t k = 0; k < toks.size(); ++k) {
            unsigned target = base + (unsigned)k;
            int oi = findArgObjectIndex(out, target);
            if (oi < 0) continue;
            writeCStringToObject(out[(size_t)oi].second, toks[k]);
          }
        }
      }
      ...
      if (!kTest_toFile(&b, getOutputFilename(getTestFilename("ktest", id)).c_str())) {
        ...
        ++m_numGeneratedTests;
        if (WriteArgConds) {
          std::map<unsigned, std::vector<std::string>> perArg;
          const unsigned GLOBAL = std::numeric_limits<unsigned>::max();
          static const std::regex re(R"(arg(\d+))");
          ...
          for (auto it = state.constraints.begin(); it != state.constraints.end(); ++it) {
            ...
            while (std::regex_search(searchStart, s.cend(), m, re)) {
              unsigned idx = (unsigned)std::stoul(m[1].str());
              perArg[idx].push_back(s);
              matched = true;
              searchStart = m.suffix().first;
            }
            if (!matched) perArg[GLOBAL].push_back(s);
          }

          for (const auto &kv : perArg) {
            std::string suffix = (kv.first == GLOBAL)
                                  ? "argGLOBAL.conds"
                                  : ("arg" + std::to_string(kv.first) + ".conds");
            auto f = openTestFile(suffix.c_str(), id);
            if (!f) continue;
            *f << "# test_id=" << id << "\n";
            if (kv.first == GLOBAL) {
              *f << "# constraints with no explicit arg<N> reference\n\n";
            } else {
              *f << "# argv index hint: arg" << kv.first
                  << " (usually corresponds to argv[" << (kv.first + 1) << "])\n\n";
            }
            for (const auto &line : kv.second) *f << line << "\n";
          }
        }
      }

      for (unsigned i=0; i<b.numObjects; i++) delete[] b.objects[i].bytes;
      delete[] b.objects;
    }
...
int main(int argc, char **argv, char **envp) {
  ...
  if (!ArgScoreFile.empty()) {
    std::ifstream fin(ArgScoreFile);
    if (!fin) {
      klee_error("Unable to open arg-score file: %s", ArgScoreFile.c_str());
    }
    std::string line;
    while (std::getline(fin, line)) {
      ...
      double score = std::stod(parts.back());
      parts.pop_back();
      std::string key;
    ...
    klee_message("Loaded %zu argument scores", ArgScores.size());
  }
