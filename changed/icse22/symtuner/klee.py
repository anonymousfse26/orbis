...
class KLEE(SymbolicExecutor):
    ...
    def run(self, target, parameters, seeds, arguments, test_dir, pgm, budget, **kwargs):
        ...
        if sym_args != "":
            if seed_cmd != "":
                cmd = [str(self.bin), seed_cmd, *klee_options, f"-arg-score-file={test_dir}/{pgm}.score", f"-seed-time={budget // 4}", str(target),
                    " ".join(arguments), sym_args, *sym_files_options, *sym_stdin_options, *sym_stdout_options]
            else:
                cmd = [str(self.bin), *klee_options, f"-arg-score-file={test_dir}/{pgm}.score", f"-seed-time={budget // 4}", str(target),
                    " ".join(arguments), sym_args, *sym_files_options, *sym_stdin_options, *sym_stdout_options]
        else:
            cmd = [str(self.bin), *klee_options, f"-arg-score-file={test_dir}/{pgm}.score", f"-seed-time={budget // 4}", str(target),
